...
from .rms_microcanonical_kinetic_modeling import RMSMicrocanonicalKineticModelingJobFactory
...
class KineticModeling(Gear):
    ...
    class Options(Gear.Options):
        ...
        __slots__ = (
            ...
            "flux_variance_label",
            "microcanonical_available_energy",
            "microcanonical_energy_tolerance",
            "microcanonical_active_rotors",
            "eckart_tunneling"
        )
        ...
        def __init__(self) -> None:
            ...
            """
            self.microcanonical_available_energy: Optional[float] = None
            """
            Optional[float]
                The energy for which RRKM rate constants will be calculated.
            """
            self.microcanonical_energy_tolerance: float = 40.0 * utils.HARTREE_PER_KJPERMOL
            """
            float
                Reactions are considered accessible to the microcanonical microkinetics if the TS energy is lower than
                the available energy plus this tolerance (in Hartree).
            """
            self.microcanonical_active_rotors: bool = False
            """
            bool
                If true, the energy is not only distributed to classical harmonic oscillators but also
                to the rigid rotor degrees of freedom (calculation of RRKM rate constants).
            """
            self.eckart_tunneling: bool = False
            """
            bool
                If true, use the eckart tunneling model (only implemented for microcanonical kinetic modeling).
            """
    options: Options
    ...
    def _get_job_factory(self) -> KineticModelingJobFactory:
        ...
        if self._job_factory is None:
            if self.options.job == RMSKineticModelingJobFactory.get_job():
                if self.options.microcanonical_available_energy is None:
                    print("Constructing RMS kinetic modeling job")
                    ...
                    print("Constructing RMS kinetic modeling job assuming microcanonical rate constants")
                    self._job_factory = RMSMicrocanonicalKineticModelingJobFactory(
                        self.options.model_combinations,
                        self.options.model_combinations_reactions,
                        self._manager,
                        self._get_energy_references(),
                        self.options.microcanonical_available_energy,
                        self.options.microcanonical_energy_tolerance,
                        self.options.microcanonical_active_rotors,
                        self.options.eckart_tunneling,
                        self.options.uncertainty_estimator)
            elif self.options.job == KinetxKineticModelingJobFactory.get_job():
                print("Constructing Kinetx kinetic modeling job")
    ...
    def start_conditions_fulfilled(self) -> bool:
    ...
    def _loop_impl(self):
        ...
        if not self.start_conditions_fulfilled() or self.have_to_stop_at_next_break_point():
            return
